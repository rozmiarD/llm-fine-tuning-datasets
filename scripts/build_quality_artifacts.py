...
DATA=ROOT/'datasets/debian-admin-bash/debian-admin-bash-sft.v1.1.jsonl'
...
multi=multi[:40]
(EVAL_DIR/'debian-admin-bash-multiturn-eval.v0.1.jsonl').write_text('\n'.join(json.dumps(x,ensure_ascii=False,separators=(',',':')) for x in multi)+'\n')
...
lines=['# Quality audit: debian-admin-bash-sft.v1.1','','## Summary','',f'- Records: {len(recs)}',f'- User prompts with text/output blocks: {user_blocks}',f'- Refusal records: {refusals}',f'- Held-out eval records added separately: {len(single)+len(multi)}','','## Distribution','','### Subdomain']
...
(VAL_DIR/'debian-admin-bash-sft.v1.1.quality-audit.md').write_text('\n'.join(lines)+'\n')
