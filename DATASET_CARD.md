# Dataset Cards

This file is an index of dataset cards in this repository.

Dataset-specific cards should live next to their dataset documentation. The root file should stay general and should not duplicate detailed per-dataset documentation.

## Available dataset cards

| Dataset | Card | Purpose | Status |
|---|---|---|---|
| `terminal-admin-bash-master__4b-coder-instruct__sft__en__debian-ubuntu__v0.1` | [Dataset card](datasets/terminal-admin-bash-master/DATASET_CARD.md) | Debian/Ubuntu terminal administration for 4B coder-instruct models | Bootstrap sample; full corpus prepared separately |

## Card expectations

Each dataset card should describe:

- dataset name;
- status;
- intended use;
- language;
- platform;
- target model profile;
- source format;
- model-specific export notes;
- recommended answer style;
- quality priorities;
- known limitations.
