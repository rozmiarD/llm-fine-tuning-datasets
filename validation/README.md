...
The governed v0.5 Debian-admin Bash dataset uses `meta.dataset_version="0.5"`, but it still validates against `schemas/debian-admin-bash.v0.2.schema.json` because the governed record shape did not change.
...
| [debian-admin-bash v0.5 governed SFT validation report](debian-admin-bash-sft.v0.5.validation-report.md) | Debian/Ubuntu admin Bash SFT dataset validation | pass |
