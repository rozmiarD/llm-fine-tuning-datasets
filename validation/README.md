...
The governed v0.7 Debian-admin Bash dataset uses `meta.dataset_version="0.7"`, but it still validates against `schemas/debian-admin-bash.v0.2.schema.json` because the governed record shape did not change.
...
| [debian-admin-bash v0.7 governed SFT validation report](debian-admin-bash-sft.v0.7.validation-report.md) | Debian/Ubuntu admin Bash SFT dataset validation | pass |
