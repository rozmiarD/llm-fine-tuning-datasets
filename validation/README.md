...
The governed v0.6 Debian-admin Bash dataset uses `meta.dataset_version="0.6"`, but it still validates against `schemas/debian-admin-bash.v0.2.schema.json` because the governed record shape did not change.
...
| [debian-admin-bash v0.6 governed SFT validation report](debian-admin-bash-sft.v0.6.validation-report.md) | Debian/Ubuntu admin Bash SFT dataset validation | pass |
