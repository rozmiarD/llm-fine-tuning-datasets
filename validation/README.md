...
The governed v1.0 Debian-admin Bash dataset uses `meta.dataset_version="1.0"`, but it still validates against `schemas/debian-admin-bash.v0.2.schema.json` because the governed record shape did not change.
...
| [debian-admin-bash v1.0 governed SFT validation report](debian-admin-bash-sft.v1.0.validation-report.md) | Debian/Ubuntu admin Bash SFT dataset validation | pass |
