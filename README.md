...
|---|---:|---|---|---|---:|---|---|
| [`debian-admin-bash-sft.v1.0`](datasets/debian-admin-bash/README.md) | small Debian-admin | SFT | English | Debian/Ubuntu | 2552 | Debian/Ubuntu admin Bash SFT records with Bash tooling, incident triage, structured parsers, SQLite, defensive admin, risk metadata, and quality gates | governed draft dataset |
...
python validation/validate_dataset.py \
  ...
  --schema schemas/debian-admin-bash.v0.2.schema.json \
  --report validation/debian-admin-bash-sft.v1.0.validation-report.md
```
...
A dataset content version such as `v1.0` does not require a new schema when the governed source-record shape is unchanged. The current governed record-shape schema is:
...
└── validation/
    ...
    └── debian-admin-bash-sft.v1.0.validation-report.md
```
